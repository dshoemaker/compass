...
class SassExtensionsTest < Test::Unit::TestCase
  ...
  def test_math_functions
    ...
    assert_equal 0.0, evaluate("sin(pi())").to_f
    assert_equal "1",   evaluate("sin(pi() / 2)")
    assert_equal 0.0,   evaluate("sin(180deg)").to_f
    ...
    assert_equal "0.0".to_f,   evaluate("cos(pi() / 2)").to_f
    assert_equal "0.0".to_f,  evaluate("cos(3* pi() / 2)").to_f
    assert_equal "0.0".to_f,  evaluate("tan(pi())").to_f
    assert_equal "0.46365",  evaluate("atan(0.5)")
    assert_equal "0.0".to_f, evaluate("tan(360deg)").to_f
